...
# define ASVs tagged as important by boruta
important_ASVs<-filter(attStats(fixed_boruta_flower_objt), decision=="Confirmed")%>%
    rownames()

#get boruta stats of ASVs confirmed to be important
rf_importance_byOTU<-filter(attStats(fixed_boruta_flower_objt), decision=="Confirmed")%>%
    rownames_to_column(var = "otu_id")
...
## 7-  Visualize the flower features on a heat tree 
...
## 8 - Calcualte precisiona nd kappa for boruta flowers
...
# plot confusion matrix
predict(model_borutized_flower, Boruta_input_flower[,-1])
confusionMatrix(model_borutized_flower)
...
##5b - are differentially abudnant ASVs also RF importnat ASVs?

```{r}

# check a subset, there are many ways o do this
