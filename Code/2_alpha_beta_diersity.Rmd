...
calculated_diversity<-microbiome::diversity(physeq_goodSamples_CSS)

n_sequences<-sample_sums(physeq_goodSamples_CSS)
...
calculated_diversity_df<-as(sample_data(merge_phyloseq(physeq_goodSamples_CSS,merg_to_ps)),"data.frame") # forces sample data of updated phyloseq object into a dataframe

#Lets see this on a simple boxplot for Shannon diversity index
ggplot(calculated_diversity_df, aes(x =Treatment, y = fisher, fill =Treatment ))+
  geom_boxplot()+
  theme_bw()+
...
leveneTest((shannon) ~ Treatment, data = calculated_diversity_df) 
...
HSD.test(y = anova_shannon, "Treatment", group=TRUE, console=TRUE)
LSD.test(y = anova_shannon, "Treatment", group=TRUE, console=TRUE)
